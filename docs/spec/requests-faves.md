...
Search and favorites paginate like news: **no list scroll**. Fill the pane with as many rows as fit in **normal viewing** (phone portrait, or the tablet two-pane; rotation must not change the count). UI pages slice a flat index over the server pages (search 20 / faves 100), stitching the next server page when a window crosses a boundary so only the true last leftover is short. Bottom bar, last page > 1: previous `<` and next `>` stay pinned at the bar edges; first page, a sliding window of nearby pages, one `...` (jump-to-page), last page sit in the middle. Do not put `...` on both sides. Page numbers are one line (never stack digits). Page numbers use a width for the last page’s digit count so 9→10 does not shift prev/next — measure that slot from the pager typeface (widest digit × count), not a dp-per-character guess. Jump-to-page accepts at most that many digits. Search `last_page` / `total` are in the JSON. A past-last search `page=` can be `total: 0` / empty — do not store it or treat it as last. UI last page is leftover of `total` (or `(last_page-1)*20 + last page length` once that page is loaded). A last UI window must not request a server page past `last_page`. Faves last page: HTML pagination (above), leftover-trimmed against the previous JSON page. Load server page 1 and that leftover last page before showing how many UI pages exist. Clamp faves windows to that leftover last too.
