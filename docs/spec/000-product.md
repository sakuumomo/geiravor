...
- Songs tab: last played (5) and queue (5) from `timestamp`, relative times. Hide queue when `isafkstream == false`. Mark `type == 1` as `/r/`. Always tabs (Now Playing \| Songs \| Settings), any width.
