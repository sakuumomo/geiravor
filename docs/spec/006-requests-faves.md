...
Search and faves **listing** stay off disk. Process RAM holds fetched server pages so paging the same query/nick does not GET again (UI page 1 still re-GETs after showing RAM). **Membership** (`tracks_id` + `meta`) is Room + RAM for the nicks the user **typed and committed**: Favorites tab nick (IME Done, or the first nonempty — “remember nick”) and Settings → Connection nick. Heart / catalog match those nicks so a fave is detected before the list GET. After a successful IRC fave/unfave, GET `/faves` for those nicks (IRC nick first) and set the heart from that plus the in-process overlay — a successful **fave** still fills if the GET has not caught up. Do not let a membership poll that started before the tap unfill it. Revalidate on start and after a successful fave/unfave. Persist membership **only if the rows actually changed** (`DiskPolicy.changed`, `001-architecture.md`). Peeking another nick in the Favorites field GETs that list only and is **never** written. Changing a committed nick drops the previous nick’s membership (unless it is still the other committed field). Request random GETs live on tap.
