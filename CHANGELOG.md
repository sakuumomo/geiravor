...
- News list flavor no longer leaks `message-body` class names. Hug panes cap to the tab so Staff/Schedule can scroll instead of clipping. Auto AFK progress uses the `/api` song window. About is display-only. Unselected section chips use the raised border fill. Favorites Request honors can-request; Request random leftover-trims. Heart membership follows committed nicks. Alarm Snooze is omitted when snooze is off. Empty news comments fail visible. Holiday sniff can run again after an hour in-process. Session Play enters FGS. DJ/fave WorkManager starts after prefs load. Hanyuu titles with apostrophes parse; “already” is not a bare substring. Backup of secrets is off.
- Live DJ `/api` may send `tags`/`queue`/`lp` as JSON `null` (no catalog track, unlike Hanyuu). That parse must not fail or the last AFK paint sticks. Leftover AFK `start_time`/`end_time`/`trackid` are ignored for the clock and fave-on-air key. Live DJ Now Playing tags come from ICY `Icy-Tags` when `/api` tags is empty. Pager shows `…` after 1 on the last pages. News `#id` inserts `>>id` at the caret and leaves the caret after it. Vol − / Vol + are a speaker with minus / plus.
