# 013 — Schedule and staff (0.3.0)

HTML only. No JSON. Not in Android Auto. Parse in `core/` (news-list style: look for known markers, not a general HTML client, not a WebView).

Constants (Rust / Policy, not UI literals): `https://r-a-d.io/schedule`, `https://r-a-d.io/staff`, weekdays Monday-first, staff role order `staff` \| `dev` \| `dj`, source zone `America/New_York`.

## Fetch

GET **when that Board section is opened** (lazy), then revalidate on open. Not on a timer. Show Room first; write only if `DiskPolicy.changed` (`001-architecture.md`). If it changed, update the screen; if it is the same, do not write and do not flash loading.

Theme name on this HTML (`/assets/{name}/css/`) is reused for holiday sniff before a dedicated `GET /` (`002-api.md`, `005-ui.md`).
...
Settings → General **Convert schedule times to local** (default **off**, `008-settings.md`). When off, show the stored body. When on, rewrite **unambiguous** clock tokens **at paint** (Room keeps the original). Convert **in place** — do not append “for You”.
...
Submit, help, Auto schedule/staff, holiday packs on Auto, site login, `/v1/sse`.
