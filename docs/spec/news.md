...
The article is the `message-body` HTML (paragraphs, `<br>`, `<em>`, `<strong>`, `<img>`, `<time>`), not the list flavor. Skip the relative `data-type="medium"` timeago line. **← News** stays pinned at the top as a link over the scrolling article — not a toolbar or rule. A compact theme **film** chip sits behind the label: **transparent at rest**, opaque on hover (glass 50% black, Christmas white sheet, Default `surface`) so the pointer hover reads. The body and comments scroll under it. Extra pad between the article body and the first comment.
...
- **`#id`** (the comment id) quotes `>>id\n` into the composer (append; keep a trailing newline). If the composer already has text that does not end in a newline, insert a space before `>>`.
