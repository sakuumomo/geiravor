...
| Destination | Content |
|---|---|
| Now Playing | Play/stop, volume 0–100 (default 80), **Mute**, **Fave** (heart), title/artist, collapsible tags, progress + mm:ss / mm:ss (AFK only; live DJ hides the bar), labeled listener count (centered; `N listeners`, singular `1 listener`), DJ image + `dj.djname`, **Next** then **Last played** (centered; live DJ next is `???`), then thread if present. Toggle the heart **immediately** on tap; revert if the attempt fails or is a no-op. Queue extra fave/unfave taps while one IRC session is in flight; after three taps in that window, ignore extras. Failures also as red text, faded out when `np` changes |
...
Follow the live homepage order for player chrome: play + volume + mute, title, tags (+/-), progress + listeners + clock, DJ column, next, previous, then thread.
