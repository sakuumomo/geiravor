...
| Settings | Section tabs **General \| Auto \| Connection**. Overflow **Alerts** (alarm / snooze / sleep / DJ notifier) — not a fifth bottom tab. Connection is Rizon vs Bouncer IRC (`006-requests-faves.md`, `008-settings.md`): required labels use `*` (Rizon: Nick; Bouncer: Nick and Host); password and client-key fields are masked with no copy/cut; client cert may copy; Clear+confirm on each PEM; Test connection shows the server SHA-256 |
