...
Search **listing** stays off disk. Process RAM holds fetched search/faves server pages so paging the same query/nick does not GET again (UI page 1 still re-GETs after showing RAM). **Committed** faves listing (Favorites nick and Connection nick — one copy when they are the same) is Room + RAM: hydrate into RAM at process start, show that cache first when opening a page, GET, write the page only if `DiskPolicy.changed`, then keep RAM. Peeks stay HTTP + RAM. After page 1, GET the leftover last server page so UI last page is the true leftover count — do **not** assume the last JSON page is 100. If leftover trim drops `last_page`, GET that new last before painting the pager. **Membership** (`tracks_id` + `meta`) is Room + RAM for the nicks the user **typed and committed**: Favorites tab nick (IME Done, or the last nick whose `/faves` GET succeeded — “remember nick”) and Settings → Connection nick. Heart / catalog match those nicks so a fave is detected before the list GET. After a successful IRC fave/unfave, GET `/faves` for those nicks (IRC nick first) and set the heart from that plus the in-process overlay — a successful **fave** still fills if the GET has not caught up. Do not let a membership poll that started before the tap unfill it. Revalidate on start and after a successful fave/unfave. Persist membership **and** committed listing pages **only if the rows actually changed** (`DiskPolicy.changed`, `001-architecture.md`). A typed nick that is not the remembered one is a peek (GET that list only, no membership or listing write) until IME Done or a successful `/faves` GET for that same typed nick; then it **replaces** the remembered Favorites nick. Do not remember the Connection fallback when the field is empty. A failed GET does not replace the remembered nick. Changing a committed nick drops the previous nick’s membership and listing pages (unless it is still the other committed field). Request random GETs live on tap.
