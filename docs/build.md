...
Pinned Nix installer (not `@main`). Two required jobs: **`rust`** (`nix develop .#rust` — fmt, clippy `-D warnings`, `cargo test`, rustdoc `-D warnings`, `scripts/check-pins`, `cargo deny check`) and **`android`** (Gradle unit tests and `assembleDebug`). Tag `v*` also builds and uploads the four GitHub APKs. `merge_group` is the merge-queue event. Free unused runner Android/.NET/etc. so the SDK fits. No emulator, no DHU, no Icecast GET, no live `/api` or Rizon. Dependabot: Actions, Cargo, Nix (`flake.lock`). rustdoc deploys to GitHub Pages from `main` (not the wiki). PR titles must be Conventional Commits (`pr-title` workflow).
